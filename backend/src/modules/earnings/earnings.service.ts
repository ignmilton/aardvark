...
import { MIN_PAYOUT_AMOUNT } from "@aardvark/shared";
...
export class EarningsService {
  ...
  async getSummary(
    ...
    for (const earning of earnings) {
      earningsByType[earning.type] =
        (earningsByType[earning.type] || 0) + earning.netAmount;
    }
  ...
  async updateAccountStatus(stripeAccountId: string): Promise<void> {
    ...
    const stripeAccount =
      await this.paymentsService.getConnectAccount(stripeAccountId);
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!account || !account.payoutsEnabled) {
        throw new BadRequestException(
          "Payout account not set up or not verified",
        );
      }
