...
import { AuthorEarning, AuthorPayoutAccount, Payout, User } from '@/database/entities';
...
describe('EarningsService', () => {
  ...
  const mockEarnings = [
    { id: 'e1', grossAmount: 100, platformFee: 30, netAmount: 70, type: 'story_unlock' },
  ...
  describe('getEarnings', () => {
    it('should return paginated earnings', async () => {
      const result = await service.getEarnings('author-123', { page: 1, limit: 20 });
  ...
  describe('setupPayoutAccount', () => {
    ...
    it('should create new Stripe Connect account if none exists', async () => {
      ...
      accountRepo.findOne.mockResolvedValue(null);
      paymentsService.createConnectAccount.mockResolvedValue({ id: 'acct_new' } as any);
      accountRepo.create.mockReturnValue({ id: 'account-new', stripeConnectAccountId: 'acct_new' } as any);
      accountRepo.save.mockResolvedValue({ id: 'account-new', stripeConnectAccountId: 'acct_new' } as any);
      paymentsService.createConnectAccountLink.mockResolvedValue({ url: 'https://onboarding.stripe.com' } as any);
  ...
  describe('requestPayout', () => {
    it('should throw BadRequestException if account not set up', async () => {
      ...
      await expect(service.requestPayout('author-123')).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if payouts not enabled', async () => {
      const mockManager = {
        getRepository: jest.fn().mockReturnValue({
          findOne: jest.fn().mockResolvedValue({ ...mockPayoutAccount, payoutsEnabled: false }),
        }),
      };
      dataSource.transaction.mockImplementation((cb: any) => cb(mockManager));
  ...
  describe('processPayout', () => {
    it('should throw BadRequestException if payout not found', async () => {
      ...
      await expect(service.processPayout('invalid-payout')).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if payout not pending', async () => {
      payoutRepo.findOne.mockResolvedValue({ id: 'payout-123', status: 'completed' } as any);

      await expect(service.processPayout('payout-123')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should mark payout as failed on error', async () => {
      ...
      payoutRepo.findOne.mockResolvedValue(pendingPayout as any);
      accountRepo.findOne.mockResolvedValue(mockPayoutAccount as any);
      paymentsService.createTransfer.mockRejectedValue(new Error('Transfer failed'));
      payoutRepo.save.mockImplementation((p) => Promise.resolve(p as any));

      await expect(service.processPayout('payout-123')).rejects.toThrow('Transfer failed');
    });
  });
