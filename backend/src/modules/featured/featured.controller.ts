...
export class FeaturedController {
  ...
  async getHero(@Query('limit') limit?: number) {
    return this.featuredService.getByPlacement(FeaturedPlacement.HOMEPAGE_HERO, limit || 1);
  }
  ...
  async getCarousel(@Query('limit') limit?: number) {
    return this.featuredService.getByPlacement(FeaturedPlacement.HOMEPAGE_CAROUSEL, limit || 10);
  }
  ...
  async update(
    @Param('id') id: string,
    @Body() dto: UpdateFeaturedContentDto,
  ) {
