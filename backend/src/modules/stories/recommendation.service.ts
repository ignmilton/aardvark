...
import { Story, ReaderProgress, Rating, User } from "@/database/entities";
  ...
  private async getCollaborativeRecommendations(
    ...
    try {
      ...
      if (excludeIds.length > 0) {
        queryBuilder.andWhere("rating.storyId NOT IN (:...excludeIds)", {
          excludeIds,
        });
      }
      ...
      return recommendations
        ...
        .map((r) => ({
          ...
          score:
            parseFloat(r.avgRating) * 0.8 +
            Math.min(parseInt(r.ratingCount) / 10, 1) * 0.2,
    ...
    try {
      ...
      for (const rating of userRatings) {
        if (rating.story) {
          ...
          const currentCatWeight =
            categoryPreferences.get(rating.story.category) || 0;
          categoryPreferences.set(
            rating.story.category,
            currentCatWeight + weight,
          );
      ...
      for (const progress of userProgress) {
        if (progress.story) {
          ...
          const currentCatWeight =
            categoryPreferences.get(progress.story.category) || 0;
          categoryPreferences.set(
            progress.story.category,
            currentCatWeight + weight,
          );
      ...
      const queryBuilder = this.storyRepository
        ...
        .andWhere("story.category IN (:...categories)", {
          categories: topCategories,
        });

      if (excludeIds.length > 0) {
        queryBuilder.andWhere("story.id NOT IN (:...excludeIds)", {
          excludeIds,
        });
      }
