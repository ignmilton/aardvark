...
import { Repository, FindOptionsWhere, Not, IsNull, MoreThan } from "typeorm";
import {
  StoryStatus,
  StoryQueryParams,
  CreateStoryDto,
...
export class StoriesService {
  ...
  private async generateUniqueSlug(title: string): Promise<string> {
    ...
    // Check-then-insert has a race window; the unique index on slug
    // will catch collisions, and the caller should retry on conflict.
  ...
  async findFeatured(limit = 4): Promise<Story[]> {
    ...
    const topRated = await this.storyRepository
      ...
      .andWhere(featuredIds.length > 0 ? "story.id NOT IN (:...ids)" : "1=1", {
        ids: featuredIds,
      })
  ...
  async findTrending(limit = 6): Promise<Story[]> {
    ...
    const popular = await this.storyRepository
      ...
      .leftJoinAndSelect("story.author", "author")
      .where("story.status = :status", { status: StoryStatus.PUBLISHED })
      .andWhere(recentIds.length > 0 ? "story.id NOT IN (:...ids)" : "1=1", {
        ids: recentIds,
      })
  ...
  async update(
    ...
    if (updateDto.coverImageUrl !== undefined)
      story.coverImageUrl = updateDto.coverImageUrl;
    if (updateDto.category !== undefined) story.category = updateDto.category;
    if (updateDto.tags !== undefined) story.tags = updateDto.tags;
    if (updateDto.contentWarnings !== undefined)
      story.contentWarnings = updateDto.contentWarnings;
    if (updateDto.collaborationMode !== undefined)
      story.collaborationMode = updateDto.collaborationMode;
    if (updateDto.status !== undefined) story.status = updateDto.status;
    if (updateDto.isPremium !== undefined)
      story.isPremium = updateDto.isPremium;
    if (updateDto.creditCost !== undefined)
      story.creditCost = updateDto.creditCost;
  ...
  async submitForReview(id: string, userId: string): Promise<Story> {
    ...
    if (story.authorId !== userId) {
      throw new ForbiddenException(
        "You can only submit your own stories for review",
      );
    }
    ...
    if (!story.rootSegmentId) {
      throw new ForbiddenException(
        "Story must have a root segment before submitting for review",
      );
    }
  ...
  async approveStory(
    ...
    if (story.status !== StoryStatus.PENDING_REVIEW) {
      throw new ForbiddenException(
        "Only stories pending review can be approved",
      );
    }
  ...
  async rejectStory(
    ...
    if (story.status !== StoryStatus.PENDING_REVIEW) {
      throw new ForbiddenException(
        "Only stories pending review can be rejected",
      );
    }
  ...
  async requestChanges(
    ...
    if (story.status !== StoryStatus.PENDING_REVIEW) {
      throw new ForbiddenException(
        "Only stories pending review can have changes requested",
      );
    }
  ...
  async getTranslations(
    id: string,
  ): Promise<{ original: Story | null; translations: Story[] }> {
    const story = await this.findOne(id);
  ...
  async createTranslationLink(
    ...
    if (
      ...
      throw new ForbiddenException(
        "You can only link translations for your own stories",
      );
    }
    ...
    if (original.originalStoryId) {
      throw new ForbiddenException(
        "Cannot link to a story that is itself a translation",
      );
    }
  ...
  async removeTranslationLink(
    ...
    if (
      ...
      throw new ForbiddenException(
        "You can only unlink translations for your own stories",
      );
    }
