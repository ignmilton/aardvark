...
describe("CreditsService", () => {
  ...
  describe("addCreditsFromPurchase", () => {
    it("should add credits from bundle purchase", async () => {
      ...
      const result = await service.addCreditsFromPurchase(
        "user-123",
        "bundle-123",
        "pi_123",
      );

      expect(bundleRepo.findOne).toHaveBeenCalledWith({
        where: { id: "bundle-123" },
      });
  ...
  describe("unlockStory", () => {
    it("should throw NotFoundException if story not found", async () => {
      ...
      await expect(
        service.unlockStory("user-123", { storyId: "invalid" }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("should throw BadRequestException if story is not premium", async () => {
      ...
      await expect(
        service.unlockStory("user-123", { storyId: "story-123" }),
  ...
  describe("claimDailyBonus", () => {
    it("should throw BadRequestException if already claimed today", async () => {
      ...
      await expect(service.claimDailyBonus("user-123")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("rewardStoryCompletion", () => {
    it("should throw BadRequestException if already rewarded", async () => {
      ...
      await expect(
        service.rewardStoryCompletion("user-123", "story-123"),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw NotFoundException if story not found", async () => {
      ...
      await expect(
        service.rewardStoryCompletion("user-123", "invalid-story"),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("getBundles", () => {
    it("should return active bundles sorted by price", async () => {
      const bundles = [
        mockBundle,
        { ...mockBundle, id: "bundle-2", priceInCents: 999 },
      ];
